...
こちらを使うと、1つのコマンドでts-nodeコンパイル後に各種コマンドが実行されるようになります。
...
なお、マイグレーションファイルとして指定したものがそのまま使われておらず、タイムスタンプ(Unixタイム)がプレフィックスとして追加されます。
...
今回はCLI(`migration:run`)からマイグレーションを実行していますが、データソースオプションで`migrationsRun`をtrueに指定すると、アプリケーション実行時にマイグレーションも同時に実行してくれます。
:::
...
初期構築で利用した`InitialSchema1677030260510`は適用済み(`[X]`)で、先程ロールバックした`AddTag1677034161321`は未適用(`[ ]`)であることが分かります。
