...
`
    ...
    if (!safeResponse(result) || !safeResponse(target)) return ""

    return `<div>
    <fieldset class="chatgpt-jargon">
      <legend><img alt="mameka" src="/img/logo/mameka6_30.png">豆香の豆知識(generated by GPT-3)</legend>
      <div>今日の言葉: ${target}</div>
      <div>${result.replaceAll(/\r?\n/g, '<br />')}</div>
      <div><b>※本コラムはOpenAIのChat APIで自動生成したものです。内容に誤りがある可能性がありますのでご注意ください。</b></div>
    </fieldset>
  </div>`;
  } catch (e) {
    console.log({ e });
...
function safeResponse(inputString) {
  const patterns = [
    /<script[^>]*>/i,
    /<\/script>/i,
    /<[^>]+on\w+=/i,
