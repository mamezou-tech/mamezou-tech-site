...
また、テンプレートの各要素には`data-testid`を追加して、テストコードでルックアップできるようしています(`testing-library`のユーティリティメソッドを利用します)。
...
最初に、ブラウザ独自の機能に依存しないシンプルなテストを記述してみます。
...
コードを見れば分かるように、実ブラウザを使ったテストですので、セッションストレージは本物でモック化する必要はなく、シンプルで直感的です。
...
CypressのE2Eテスト同様に、テスト結果は非常に見やすくデバッグも簡単です。
