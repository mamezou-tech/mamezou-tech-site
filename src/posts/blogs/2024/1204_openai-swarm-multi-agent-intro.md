...
2025年開花しそうなAI注目技術と言えばエージェント機能が挙げられます[^1]。

[^1]: エージェント機能の定義ははっきりとしていませんが、ここではAIが人間に代わってタスクを再起的に実行して目的を達成してくれるものとします。

最近でも、Anthropicが[Computer Use](https://docs.anthropic.com/en/docs/build-with-claude/computer-use)と呼ばれるAIでPCを直接操作するAIエージェントを公開しました。
現時点ではベータ版ですが、Anthropic APIの他、AWS BedrockやGoogle Cloud Vertexからも利用できます(Claude 3.5 Sonnetモデル)。
OpenAIやGoogle等のAIベンダーでも自律型のエージェント機能を順次リリースする予定とのことで大きな期待(と懸念)を集めています。

ここでは、そんな期待のAIエージェントを協調動作(エージェントオーケストレーション)させる仕組みを整理したいと思います。
これを実現する方法は色々と考えられると思いますが、今回はOpenAI Cookbookの以下記事で言及している方法を深掘りしてみます。

- [OpenAI Cookbook - Orchestrating Agents: Routines and Handoffs](https://cookbook.openai.com/examples/orchestrating_agents)

本記事のタイトルで使っているSwarmは、この記事で言及している方法のサンプル実装として公開されているものです(もちろんDocker Swarmとは別物ですw[^2])。

[^2]: Docker Swarmとは全く別物ではありますが、Docker Swarmはコンテナ、OpenAIのSwarmはエージェントを協調動作させるものなので、対象が違うだけで意味合いは同じですね。

- [GitHub openai/swarm](https://github.com/openai/swarm/tree/main)

本レポジトリは上述の記事のためのサンプルソースコードの扱いのようですが、エージェントオーケストレーションの考え方を学ぶ上で有用なものになっています。

## Swarmのコンセプト

Swarmは非常にシンプルで、以下2つのコンセプトで構成されています。

## Routines / Agent

Cookbookの方ではRoutinesと言われていましたが、SwarmのREADMEではAgentと言われています。本記事ではエージェントに統一します。
...
このようにエージェントは自然言語で記述した指示と、その指示を実行するツールで構成されます。
...
ここでは、`instructions`(システムメッセージ)と`functions`(Function calling)が重要なプロパティになります。
`instructions`に該当エージェントの責務とそれを達成するための指示を、`functions`に実際に指示内容を遂行するPython関数を設定します。
...
電話を他の人(エージェント)に転送するイメージとのことです。
エージェントが実行すべきタスクが自分の責任範囲を超えると判断したら、適切なエージェントを選択してバトンタッチ(引き継ぎ)することがHandoffsと言えそうです。
...
このように次々にHandoffsを繋いでいけば多数のエージェントで協調動作できます(再起的にエージェントのHandoffsを繋げば再起構造にもできます)。

Swarmでは、エージェント引き継ぎ用の関数をツールに加えることでHandoffsを実現します。
Handoffs関数は以下のようなものになります(Cookbookのサンプルをもとに修正を加えています)。

```python
# 引き継ぎ先エージェント
refund_agent = Agent(
    name="Refund Agent",
    instructions="あなたは返金エージェントです。ユーザーの要求にあったツールを実行してください。",
    tools=[...]
)
...
`transfer_to_refunds`がHandoffs関数です。`transfer_to_XXX`(XXXはエージェント名)というネーミングにすることでLLMが適切に判断してくれるとのことです。
このHandoffs関数を引き継ぎ元のエージェントのツールとして指定しておきます。
...
このトリアージエージェントはHandoffs関数のみを設定していますが、もちろん通常の関数と組み合わせても構いません。
このような形で指定しておくと、LLMはタスクを実行する代わりに適切なエージェントを選択して、そのタスクを引き継ぐことになります。
...
LLMがツール実行不要と判断する(つまり通常のメッセージを返す)まで、Chat Completion APIを呼び出しを実行し続けるシンプルなものになっています。
その中でHandoffs関数(エージェントを返す関数)が実行された場合にアクティブエージェントを切り替えていきます。
また、上図には表していませんが、Handoffs発生時はメッセージの履歴やコンテキスト情報も次のエージェントに引き継ぎ、後続エージェントが過去の情報やユーザーコンテキストを参照できる仕組みを取っています。
...
今回は旅行プラン作成アシスタント機能と題して、以下エージェントの協調動作を組み立ててみます。
...
`instructions`に具体的な指示を書かないと、エージェントに引き継ぎせずに自分勝手なレスポンスを生成してしまうことが多かったので、Handoffs関数(ツール)の実行指示を明確に記述しました。

続いて残りのバックグラウドで実行するエージェントの実装です。
...
ここでは、各エージェントは通常のタスク実行関数に加えて、旅行プラン統合エージェントへのHandoffs関数(`transfer_to_travel_planner`)も設定しています。
...
最後にこれを実行する部分です。初期エージェントとして旅行プラン統合エージェント(`travel_planner_agent`)を指定し、ユーザーの入力(出発地、目的地)を指定しています。
また、メッセージとは別にコンテキスト値(`context_variables`)を指定しています。
この情報は任意のエージェントのタスク実行関数や`instructions`で使用可能です(先ほど旅行プラン統合エージェントの`instructions`で使用していました)。

以上で終了です。これをCLIで実行すると以下のようなレスポンスが得られます。
...
さらに、Assistants APIと組み合わせることで、File SearchやCode Interpreterなど、より実用的なツールの活用が可能となり、応用の幅が広がりそうです。
...
すべてを一つにまとめた「ゴッドエージェント」を避けるため、適切な分割と設計のバランスが求められると感じました。
