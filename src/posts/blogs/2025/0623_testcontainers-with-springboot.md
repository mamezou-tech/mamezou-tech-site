...
Step3ではEnvironmentのプロパティ値を取得し、その値を`PropertiesJdbcConnectionDetails`などの接続詳細オブジェクトにバインドすることまでをAutoConfigurationがやってくれています。そして、そのバインドされたオブジェクトはDataSourceなどBeanを生成する処理で参照される流れになっています。このようにこれまでは設定ファイル（プロパティ値）に必要な設定をどう織り込むかがポイントでした。
