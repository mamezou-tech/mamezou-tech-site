...
[^7]:　例えばテスト駆動開発を採用している場合には「テスト対象の振舞いを定めてからそれを充足するように対象を変更する」開発スタイルであるため、「ある条件＝同値クラス」のテストパターン漏れは発生し難いです。この場合はNEGATE_CONDITIONALSミューテーターの利用価値は相対的に下がるかもしれません。  
