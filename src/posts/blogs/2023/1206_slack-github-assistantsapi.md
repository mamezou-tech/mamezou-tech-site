...
極力シンプルに必要最低限なサービスで構成しています。主要なロジックは2つのLambdaで実装しました。
以降でそれぞれの詳細を見ていきます。

なお、ソースコード全体は[こちらTODO]()で公開しています。
...
`lambdaClient.send`の部分が後続のLambda関数の実行部分です。ここで必要な情報をJSONに変換にして連携しています。
...
この部分は本題はありませんので、ソースコード掲載は省略します。[こちらTODO]()から参照できます。
...
これでスレッドの実行ステータスは`in_progress`に戻りポーリングに戻ります。ここでアシスタントは再度関数を実行するのかGPTから結果を取得するのかを判断することになります。
...
また、TTLを有効にして一定期間経過後に削除するようにしています[^4]。
...
コールバックURLには、先ほど控えたコールバックAPIのFunction URLを設定します。入力するとSlackが自動でURLの検証を行います。
...
ここまでくると動作確認できるはずです。その前にアプリに必要な設定情報を登録しておきます。
...
ごもっともな指摘ですね。そのままプルリクエストに優しくコメントにしてもらいましょう。
...
変更点の要約だけでなく、レビューや指摘事項の修正まで全部GPTに任せられるようになるともはや人間は要らなくなりそう。。と思った次第でした。
