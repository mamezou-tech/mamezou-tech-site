...
# まとめ
記事では仕組みを理解するためJWT認証を行う簡単なアプリを作ってみましたが、プロダクション環境でJWT認証を行う場合は、Auth0などの認証基盤サービスを使うことをお勧めします。扱っているものが認証という非常に重要な機能のため、JWT認証でウッカリや万が一があった場合のダメージが大きいため信頼のあるものを使った方が無難です。
