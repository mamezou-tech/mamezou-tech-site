...
# はじめに
2022年4月に豆蔵に入社しました、BS第二グループの中垣内（なかがいと）と申します。今回は、POSアプリの保守開発案件に携わり、AWSのS3周りの機能改修にて工夫した点をまとめ、デベロッパーサイトに初めて寄稿させていただきます。
...
# 機能要件
さて、実際の業務案件にて求められた機能要件について説明します。<br>
POSアプリケーションの、Web画面にてマスタメンテナンスを行う部分の開発を行っており、元々以下のような機能がありました。

### 既存機能
- 特定の店舗のマスタデータをS3に保管する
- 保管したマスタデータを他店舗に配信する

この機能により、新商品のデータを一括して複数の店舗にばら撒くといったことが実現されておりました。<br>
そして今回、店舗数の増大や業務形態の多様化により、マスタデータをフォルダで管理するという以下の新規機能が求められました。
...
# 案1. AWSのAPIを使用する
まずは素直にAWSのAPIを使用して実現する方法を考えました。<br>
まずはフォルダ作成ができるのか調べてみました。<br>
...
```
コマンドは実行できました。どうやら、`folder002/`というKeyで、Sizeが0のオブジェクトとして保存されるようです。<br>
...
例として、「folder003」というフォルダを「folder004」に名称変更する場合は以下のような感じです。<br>
...
無理矢理感は否めませんが、これらの方式で実装することにより、画面から見ると、マスタデータをフォルダ別に管理できているように"見せかける"ことが実現できました。<br>

ひとまず実現はできたのですが、これがベストな方式だったかと問われると疑問が残る状態です。<br>
後に同チームにてアーキテクトを担当されている田中望さんからいくつか他の実装方式を紹介していただいたので以下に記載します。

# 案3. DBにフォルダの情報を持たせる

私が開発を担当しているPOSアプリケーションでは、データベースとして[Amazon RDS](https://aws.amazon.com/jp/rds/)を使用しております。RDSにてマスタデータ管理用のテーブルを作成し、マスタデータのKeyとフォルダ名を紐付ければ、S3以外のサービスに依存することにはなるものの、フォルダ管理の仕組みが実現できるということを一案として教えていただきました。
...
余談ではありますが、S3はWebのマネジメントコンソールの画面からアクセスすると、「フォルダの作成」というボタンがありフォルダを作成することができます。（実際にはSizeが0のオブジェクトが作成されます。）<br>
また、画面上部にはパンくずリストがあり、フォルダの階層を辿ることができるようになっています。<br>
マネジメントコンソールの画面においても、実際はフォルダという仕組みは無いにもかかわらず、フォルダという仕組みがあるように見せかけているということなのだと思いますが、画面からはフォルダ管理ができるようにしているのに、APIではその方法を明示的に提供していないのには、何か意図があるのでしょうか・・・？どなたかご存じでしたら教えてください。
