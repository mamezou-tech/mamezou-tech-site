...
このため、前段に配置するApiGatewayサービスだけはFargateにすることは諦め、Elastic IP[^2]で固定IPを割り当てたEC2上にDockerをインストールし、そのDocker上でApiGatewayサービスを稼働させることにしました。この場合、ACM によるSSL/TSL証明書の発行は行えないため、無料で使える[Let's Encrypt](https://letsencrypt.org/ja/)で証明書を発行するようにしています。（CloudFrontのエンドポイントの証明書はACMで発行しています）
