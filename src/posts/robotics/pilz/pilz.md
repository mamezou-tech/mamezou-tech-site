...
この記事ではMoveItのモーションプランナーライブラリの1つであるPilz Industrial Motion Plannerを紹介します。MoveItはROSを使用してロボットのマニピュレーションを行う際に非常に便利なツールです。ただ、MoveItを使用する中で感じた課題として、1つの動作命令を行うたびにロボットが停止してしまうという点があります。複数の教示点を滑らかに繋いだ軌跡に沿って動作させる手段を調べていた時に見つけたのが今回紹介するPilzのプランナーです。
...
複数の動作命令を1つのリクエストとして受け取り、動作間で停止することなく滑らかな軌跡の生成が可能です。

従来のプランナーは、1つの動作命令ごとにリクエストが完結するため、教示点への到達のたびにロボットの動作が停止します。

また、動作間を曲線で補間でき、半径を指定することでなめらか度合いの指定が可能です。そのためマニュアルでロボットの軌跡をチューニングする作業が行いやすいことも特徴です。
...
有名なロボットであれば、{ロボットの名前}_moveit_configという名前でファイルがGithubで公開されていますし、URDFファイルから設定ファイルを生成するsetup assistantが使用できます。
...
余談ですが、こちらの動画では7秒付近で第四関節が大きく動いています。
今回はシミュレーション環境で動作させているので実行できていますが、実機では実行不可能な速度であるため、特異点を回避することが必要になります。
