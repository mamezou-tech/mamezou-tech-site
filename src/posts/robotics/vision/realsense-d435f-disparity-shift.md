...
D435fでは深度センサの最高解像度(1280x720)において最小深度距離(*)は280mmとなります[^2]。
...
今回は特徴の少ないワーク(*)をピッキングするため、D405は選定対象から除外しています。D405の評価については別の記事でご紹介したいと思います。

(*) 表面に模様や凹凸があれば画像上でこれを特徴として検出してマッチングが可能ですが、今回のワークの表面は平坦で白色(無地)でした。
