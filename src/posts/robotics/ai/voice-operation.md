...
OpenAIのRealtime APIが2024年12月18日にWebRTCをサポートし、価格も大幅に引き下げられたため、これを使って音声で自律走行ロボットを操作する機能を開発しました。どのようなアプローチでRealtime APIを組み込んだのかをご紹介いたします。
...
ロボットは産業用の清掃ロボットで前方に回転ブラシを具備しており、床の塵やゴミを履きだしながら走行します。
