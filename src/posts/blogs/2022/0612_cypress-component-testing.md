...
また、TypeScript向けにcypressディレクトリ配下には、以下のtsconfigを作成しておきます。
```json
{
  "compilerOptions": {
    "target": "es5",
...
また、テンプレートの各要素には`data-testid`を利用して、テストでルックアップできるようしています(`testing-library`のユーティリティメソッドを利用します)。
...
このように、実ブラウザを使ったテストですので、セッションストレージをモック化する必要はなく、シンプルで直感的です。
このようにブラウザ機能(セッションストレージやCookie、タブ等)に依存するコンポーネントでは絶大な力を発揮してくれそうです。
...
変更を検知する度に実行してくれますし、アプリケーションを別途起動する必要がありませんので非常に効率良くテストができそうです。
CypressのE2Eテスト同様に、テスト結果が見やすくデバッグも簡単です。

もちろんヘッドレスでの実行もできます。以下はコンポーネントテストをCLIで実行する例です。
...
CI等ではこちらを利用することになるかと思います。初回起動はそれなりに時間がかかりますが、3ケースと少ないですが、470msとテストは比較的高速です。
