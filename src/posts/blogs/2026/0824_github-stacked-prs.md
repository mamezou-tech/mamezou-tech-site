...
## はじめに
先月、GitHub のスタック PRs がパブリックプレビューになりました。

@[og](https://github.blog/changelog/2026-07-30-stacked-pull-requests-are-now-in-public-preview/)

これまでも PR のブランチからブランチを生やして、スタック PR を作るというのは普通にやってましたが、それよりもどこが便利になっているのでしょうか。見ていきたいと思います。
...
PR をスタックさせると、PR 説明にベースブランチを書いたり、中間のスタックで発生した変更を上位のスタックのブランチにリベースして反映するという作業が頻発します。スタック PRs は「特定のブランチを起点にした一連の PR スタック」に纏わる煩雑な作業を大幅に削減してくれます。
...
スタック PRs が使える環境であれば、「あ、この修正は元の PR のスコープ超えてるな。」と思ったら、ブランチを切って、`gh stack add ` でスタックに積めばいいのです。
