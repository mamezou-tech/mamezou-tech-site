...
export async function retrieveTarget(): Promise<LatestArticle[]> {
  ...
  const target = $entry.toArray().map((el) => {
    let link = "";
    let published = "";
    const linkTag = el.children.find(
      (c) => (c.type === "tag" && c.name === "id"),
    );
    if (linkTag?.type === "tag") {
      link = (linkTag?.firstChild as any).data;
    }
    const publishedTag = el.children.find(
      (c) => (c.type === "tag" && c.name === "published"),
    );
    if (publishedTag?.type === "tag") {
      published = (publishedTag?.firstChild as any).data;
    }
