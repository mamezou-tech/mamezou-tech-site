...
ここまでくると後は動かすだけです。Backstageアプリの起動はyarnコマンドで行います。
...
`Abount`ではこのコンポーネントの位置づけが分かります。ここではこのコンポーネントが誰に所有されていて(`OWNER`)、どのシステムの一部であるのかが分かります。
...
このように、このコンポーネントが何に利用されているか／何を使用しているかといった外部との依存関係がすぐに分かりますので、いろいろと調べ回る必要はなくなりそうです。
...
APIの一覧が表示され、種類(gRPC/GraphQL/OpenAPI)や現在の状態がすぐに分かります。
今回はpetstoreを選択してみます。
...
## まとめ
今回はGitHubとのOAuth認証とサンプルテンプレートからBackstageの概要を簡単に見てきました。
