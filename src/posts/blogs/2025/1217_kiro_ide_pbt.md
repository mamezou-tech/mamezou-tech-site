...
この説明は、今回の検証で非常に腑に落ちました。

従来の EBT（事例ベーステスト）は、「こういう入力ならこう動くはず」「想定されるパターンを洗い出す」という **予測可能なバグ検出**に強い一方で、**人間が思いつかなかった入力**は基本的にテストされません。

一方 PBT は、ランダムかつ多様な入力を自動生成し、「満たすべき性質」が破れるケースを探索するため、**予測できなかった種類のバグ**を検出できる可能性があります。

この違いは、後半の UI 表示テストで実感することになります。
...
以下のような要件文書が出力されました。
**受入基準が EARS 記法[^6]で明確に言語化**されています。
