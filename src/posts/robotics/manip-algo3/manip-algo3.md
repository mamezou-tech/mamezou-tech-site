...
例えば下記の図のように実際コースの$P(\tau)$にいるとき、本来コースだと取るべき本来速度$\vec{v}^{O}$で動いてしまうと、そのままずれっぱなしで移動するので次時点には予定の目標点$P'(t+\Delta t)$からさらに離れてしまう。これを避けるためには予定コースにちょうど戻るための復帰速度$\vec{v}^{reg}$分偏向しつつ移動すればよい。なお$\vec{v}^{reg}$は現時点から正確に目標点へ移動するための目標速度$\vec{v}^{tgt}$と本来速度$\vec{v}^{O}$との差である。
