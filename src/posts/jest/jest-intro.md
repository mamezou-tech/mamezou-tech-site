...
Jestは、テストランナーからマッチャー、カバレッジ等のテストに必要な一連の機能が備わっています。また、それらのセットアップが非常に簡単であることから、現在のJavaScriptで最も利用されているテストフレームワークです。
...
describeはテストをグループ化するもので任意です。任意の深さでネストして記述できます。
