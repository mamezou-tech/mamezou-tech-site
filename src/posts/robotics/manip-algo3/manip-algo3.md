...
まず、ある関節角度セット$\vec{\theta_\tau}$で構成される姿勢に対して、算出される$J^*$を
$$
J^{*}=J^*(\vec{\theta_\tau})
$$
と関数表現する。
...
とする。理論的には下記のような積分を含む式
$$
\vec{\theta}_{(t+\Delta t)}=\int_{\tau=t}^{t+\Delta t}{J^*(\vec{\theta}_{(\tau)})\vec{v}_{(\tau)}}d\tau + \vec{\theta}_{(t)}
$$
を漸化的に計算すれば、時々刻々と関節角度セットが出力され、特異点回避可能な軌跡生成が可能となる。しかし実際には、$\vec{v}_{(t)}$を前もって正確に算出することは困難である。仮に事前の軌跡生成によって算出した本来軌跡から得た本来速度$\vec{v}^O_{(t)}$を$\vec{v}_{(t)}$ と見なして新たな$\vec{\theta}_{(t)}$を得ても、特異点付近を通過するごとに本来コースからどんどん外れることとなる。これを防ぐためフィードバック制御を行って、なるべく本来コースから外れないよう下記の計算を行う。
...
ここで、$\tau$は時刻 [ $t$～$t+\Delta t$ ]の間の時刻であり、$FK(\vec{\theta})$を関節角度からTCPの座標を求める操作、つまり順運動学の操作とすると、
$$
...
以上を用いて先の積分を計算すれば良いが、この積分には一番簡単な下記の近似を使う。これによって時々刻々と関節角度セットが出力できる。
$$
\vec{\theta}_{(t+\Delta t)}={J^*(\vec{\theta}_{(t)})\vec{v}_{(t)}}\Delta t + \vec{\theta}_{(t)}
$$

なお、特異点を通過する場合は直線的コースを期待しても少しコースが横ずれしてしまうことがある。この場合は$k_0$、$w_0$の値または$V_{max}$対$W_{max}$の比を調整することによりコースのずれを調整できる。また$g_0$の値が大きいとコースが振動することに注意されたい。


## 4. 終わりに

ロボットアームの特異点とは何か、また6軸ロボットアームでの特異点回避方法について解説した。実際の実装では、これらを実機で動作させてみてパラメータ調整を行う。また本稿で述べたロボットプログラムに基づいた軌跡生成処理としての特異点回避だけでなく、ペンダントを使ったJOG操作時の特異点回避機能も別途必要である。ただしこれは本稿の理論の応用で実装可能である。さらに特異点検知アラーム機能なども本稿の理論の応用で実装可能である。
