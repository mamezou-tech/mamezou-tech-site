---
title: 食品盛り付けロボット「美膳®」のUIとコアシステム間のアーキテクチャ
author: yuki-terada
tags: [ロボット, 美膳®, Bizen, ソフトウェア設計, GUI]
date: 2026-03-23
image: true
---
...
下記の順にサーバ & クライアントを実行します。

### 1. サーバ側

ターミナルで`counter_server`のプロジェクトディレクトリに入り、サーバのDartプログラムを実行します。
...
別のターミナルを開いて`counter_client`のプロジェクトディレクトリに入り、クライアントのFlutterプログラムを実行します。
...
もしサーバが先に起動されていれば、クライアントを起動すると自動的にサーバに接続されるはずです。

クライアントの**[+1] / [-1]** ボタンをタップすると、操作ストリームがサーバに送信されてカウンタの状態が更新されます。

サーバはカウンタの状態をクライアント方向のストリームで送信して、受信したクライアントはカウンタの表示を更新します。

ここでもうひとつ別のターミナルを開いてクライアントプログラムを起動してみてください。
