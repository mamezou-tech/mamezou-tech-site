...
このように、`enforce`以外は違反してもPod自体の生成は可能です。

[^2]: Kubernetes 1.23はBetaバージョンですが、今後Stableバージョン(1.25予定)になるとラベルではなく、`spec`の方に代わると思われます。
...
とはいえ、PSS自体はKubernetesが考えるセキュリティスタンダードです。
