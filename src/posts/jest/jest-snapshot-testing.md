...
各UIコンポーネントで生成したHTMLを検査することを想定していますが、ここでは簡易的に固定のHTML文字列を比較対象とします。
...
toMatchSnapshotの引数がプロパティマッチャーです。
ここで、等価条件以外で比較したいものを記述します。上記ではランダム値のIDはUUIDフォーマットであるか、タイムスタンプはNumber型であることのみを検査するように記述します。
