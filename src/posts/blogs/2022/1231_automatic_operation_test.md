---
title: 自動打鍵テスト - Webアプリの自動テストなんだから操作も検証もテストコードの生成もできるだけ自動にしたほうがいいよね
...
WebアプリのテストをSelenium[^1]などを利用することにより、**人の手による操作（キー入力やマウスクリック）**や**人の目による確認を必要としない**意味で自動化したものをさします。

操作用コードや検証コードも自動生成したり、Jenkinsで定期的に実行させるなど、不具合の検出を自動化することまでを範囲とします。
...
public class 顧客窓口Test extends BaseTest {
  ...
  public void test窓口を検索する() {
    login("user01") // 認証の操作は共有化しておく
      ...
      .___窓口検索___()
      .担当組織検索() // 入力補助ボタンをクリックし組織コードの入力補助ダイアログを開く
...
public class 窓口検索 extends BasePageObject<窓口検索> {
  ...
  public 窓口検索 担当組織(String orgCode) {
    sendKeys(By.id("organizationCode"), orgCode);
    return this;
  }
