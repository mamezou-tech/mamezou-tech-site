...
皆さんご存知のように2020年春から新型コロナ感染症が日本で蔓延し、IT業界では自宅で仕事をするという事が当たり前になりました。前述の通り、そのうち自宅で仕事をする世の中がやってくると考えていたので自宅選定においても落ち着いて仕事ができるような間取りを十分に考慮しました。新型コロナ感染症のおかげで（せいで）早くもその効果を実感しています。
...
研修の場がオンライへ移行して3年目になります。この3年間、4月頭から7月末まで担当する新人研修で朝から晩までオンライン臨戦状態で次のよなことをしています。
...
担当するお客様により、zoom + slack + Google Workspace であったり、Teams だけで完結していたりします。研修を実施する上では次の3つが重要と考えます。
...
デスクトップPCは自作PC（AMD Ryzen 5 3500、32GB、RADEON RX560 X 2枚、有線LAN接続）です。ノートPCは会社貸与のLet's NOTE(Core i7-10810U、16GB、UHD Graphics、WiFi接続)です。

デスクは、昔々13年前に IKEA で購入した、150 cm X 75 cm のテーブルトップを使っています。今現在IKEAオフィシャルでは販売していないサイズのようです。元々はダイニングテーブル用に購入したのですが、仕事部屋が確保できるようになってからは、仕事部屋用として使っています。デスク１つでは足りないため、手作りデスクがもう１つ 85 cm x45 cm を L字構成で置いてあります。

オンライン研修の講師を担当すると、デスク上にアナログ資料を展開したり後述する卓上書画カメラでPC部品などを撮影したりするため奥行きが必要になります。更にデスク上に映像・音声用機材も設置するためとにかく広さが要求されます。

椅子に関して、これも昔々10年程前に 中古で購入した オカムラの[Baron](https://product.okamura.co.jp/ext/DispCate.do?volumeName=00001&lv0=%E3%83%81%E3%82%A7%E3%82%A2%EF%BC%8F%E3%82%BD%E3%83%95%E3%82%A1&lv2=%E3%82%BF%E3%82%B9%E3%82%AF%E3%83%81%E3%82%A7%E3%82%A2&lv3=%E3%83%90%E3%83%AD%E3%83%B3)です。
お気に入りの点は、オールメッシュ・Extra High Back です。この椅子のおかげで1日座っての研修を担当できています。
...
全景でも記述した通りオンライン研修の実施・運営にはデスクトップPCを利用しています。テレビ会議システム zoom は、10名程度のオンライン会議を実施する分には高PCスペックは要求されません。しかしオンライン研修を実施するとなると話は別です。参加者が100名を超え、更に頻繁にブレークアウトルームの機能を使ったりするとCPU性能が要求されます。ブレークアウト開始時にタスクマネージャーのCPU使用率を見てみると面白いと思います。
...
もう ATEM Mini Pro なくしてオンライン研修は成り立ちませんという位、神デバイスです。

zoom などのテレビ会議システムはPCの画面（アプリのウィンドウやデスクトップなど）を共有表示するための機能が備えられています。オンライン研修ではPDFのテキストを表示したりパワポスライドも見せたりはたまたeclipse などのIDEの画面を見せながらライブコーディングをしたりと時には手書きの図を表示したりと、表示ウィンドウの切り替えが頻繁に発生します。その切り替えの都度、zoom を操作して、共有するウィンドウを選択していては、研修のテンポが乱れますし、なにより時間が無駄になります。切り替え操作に関して、画面の共有ボタンを押す、表示したい画面を選択する、zoom が切り替わる。この一連の流れで 15秒程度かかったとします。1時間のうちに10回切り替えると、計2分半です。4%です。1日6時間とすると14分が切り替えの時間となります。もったいない。14分もあったら小さな演習1つ分位の時間が確保できてしまいますね。

時間節約のため、zoom の画面の共有では、第2カメラのコンテンツを選択し、ATEM mini の映像を選びます。

ATEM Mini へのHDMI入力は、ビデオカメラXA20の映像、デスクトップPCのマルチディスプレイ出力の１つを分配器で分けた映像、液晶ペンタブレットの映像、書画カメラの映像の計4つです。

これで、zoom の操作をともなう画面切り替えは発生せず、ATEM Mini のHDMI入力ソース切り替えボタンをポンと押すだけで一瞬で共有画面の内容を切りけることができます。

ATEM Mini の2つめのHDMI入力がPCのマルチディスプレイ②からの分岐出力になっているので、表示したいPDFやパワポのスライドをATEM Mini につながっていないディスプレイ①に表示しておき、画面共有で表示したくなった時に表示したいウィンドウをドラッグしてATEM Mini に繋がっているディスプレイ②へ移動させてあげればzoomの操作なしで迅速・簡単に画面表示できます。
...
インフラ研修でPCパーツの実物（CPUやGPU、マザーボードなど）を投影する際に活用しています。
