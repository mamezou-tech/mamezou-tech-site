...
その際にRealSenseのカメラパラメータである Disparity Shift を調整してワークディスタンス(WD)の短縮に取り組んだのでご紹介いたします。
...
## D435fの最小のワークディスタンス(WD)のカタログ値

D435fでは深度センサの最高解像度(1280x720)において最小で280mmのWDで深度が得られます[^2]。
...
以下の式[^3]の通り、理論上は深度の誤差はWDの2乗に比例します。
...
深度の要求精度はワークの形状や寸法、ハンドの機構(吸着式やチャック式)などに依存しますが、今回は比較的小さなワークを対象としたため、短い距離で撮像可能なD435fを選定しています。

[D405](https://www.intelrealsense.com/depth-camera-d405/)の方が最小のWDは100mmと短いですが、D405はプロジェクタを搭載していないパッシブステレオ方式の3Dカメラであり、特徴の少ないワークでは視差の誤差が大きくなります。
今回は無地で特徴の少ないワークを対象としたため、D405は選定対象から除外しています。D405の評価については別の記事でご紹介したいと思います。
...
- Plane Fit RMS Error[%]
  - 得られた深度(点群)で平面フィッティングを行い、各点群からこの平面までの垂線の長さで二乗平均平方根(Root Mean Square:RMS)を求めます。

今回は機材の制約で後者のRMSエラーのみを確認しました。
最小のWDは280mmですが少しマージン設けて300mmの位置とします。
...
RealSenseの深度センサは、デフォルトでは最小のワークディスタンス(MinZ)から無限遠までの深度を生成します。
Disparity Shift に値を設定することで深度を生成する最大距離(MaxZ)が小さくなり、同時にMinZも小さくなります(カタログ値より小さくできます)。
...
平らな壁を撮像対象としてカメラと壁の間の距離を調整しながら、設定したDisparity Shiftの値に対して深度が得られる限界位置(MinZとMaxZ)を調査しました。
RealSense Viewerで深度画像の半分以上の領域で深度が得られなくなった位置を限界位置とみなしました。
この時のRealSense Viewer上で表示される深度値をMinZ或いはMaxZとします。

![](https://i.gyazo.com/3ace678e507f22356fdc3ada4d0029c0.png)

結果は以下のようになりました。

![](https://i.gyazo.com/6d3ccc1700f8037ebea2f1e566c2a55c.png)

横軸がDisparity Shiftの設定値で縦軸がWDの限界値(MinZとMaxZ)です。
...
MinZは145mmですが50mm程度マージンをとって200mmのWDで計測しました。
...
WDが300mmから200mmになったことでRMSエラーが0.3mm以下となりました。
一方で80%のROIでは平面フィッティングに失敗してRMSエラーの算出に失敗しています。

DQTで表示される深度画像は左右の深度センサの内、左側の深度センサの深度画像となりますが、左端の領域では深度が得られません。
WDを300から200にしたことで深度の得られない領域が広がり、ROIの範囲内にこれが含まれてしまい平面フィッティングに失敗したようです。

![](https://i.gyazo.com/3596eb4faec7c10781680a44e503b8bb.png)

ステレオカメラは左右の深度センサの画像のそれぞれで得られた特徴(*)をマッチングして視差および深度をもとめますが、左端の領域は右側の深度センサの撮像範囲外となり深度が得られません。
また、WDが小さくなるほど撮像範囲に対して深度が得られる範囲は相対的に狭まって行きます。

(*) プロジェクタからの照射光によるランダムドット。

![](https://i.gyazo.com/832a756edb3eabfc60476e00ce952589.png)

ROIの中心位置を少し右寄りに移動できれば良いのですが、そのような機能はDQTにはないようです。

## まとめ

今回はRealSense D435fでDisparity Shiftを調整し、WDを小さくすることで深度のRMSエラーを改善しました。
