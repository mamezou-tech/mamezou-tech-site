...
:::info
REST Clientの定義ファイルはユースケースの流れやバリエーションを意識して作成すると、他への転用や、他者への説明がしやすくなります。  
筆者はユースケースの流れに沿って上から下へ定義し、バリエーションが必要な部分はリクエストボディを必要な数作成し最も代表的なケース以外をコメントにして作成しています。  
:::
...
5. 複数ユースケースのテスト実装
ひととおりのユースケースを実現しAPIの動作が安定してきたら、複数ユースケースのテストを実装します。  
...
## まとめ
複数ユースケースの流れの確認を自動化するツールとして利用すると扱いやすいです。  
動作確認時点でユースケースを意識して準備しておくことで、短時間で実装できます。テスト実行時間が短いのも魅力です。  
