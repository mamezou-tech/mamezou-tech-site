...
Go言語の標準機能としてゴルーチンという仕組みが実装されているのが特徴です。
...
実行して標準出力を確認すると、直列処理ではカウントのたびに待機しているのに対し、ゴルーチンを使った実装では同時にカウントされているのがわかります。
...
これにより、複数のゴルーチンが安全に共有メモリにアクセスしつつ、ゴルーチンによる並行処理が可能になる。
