...
今回は静的サイトのホスティングとしてS3を利用するので、パブリックアクセスを許可する必要があります。デフォルトではブロックするようになっているのでチェックを外します。
...
作成したバケットへのアクセスを許可するため、以下のバケットポリシーを設定します。`[バケット名]`の箇所を読み替えてください。
...
S3の設定が正しく出来ていれば、Origin domainの選択肢として`[作成したバケット名].s3.amazonaws.com`が表示されますので、これを選択します。選択するとNameに値がセットされます。他項目はデフォルトでOKです。
...
![a](/img/blogs/2022/1220_Route53_DNS_03.png)
