...
フロントエンド系の開発の比重は近年高くなる一方です。コンシューマー向けだけでなく、エンタープライズでも生産性向上のため良いユーザー体験が求められます。そして、移り変わりが激しくキャッチアップが大変な分野でもあります。

ここでは、定番のフレームワークの使いこなしポイントや、注目技術をご紹介します。
...
## プリレンダリング / エッジ環境
Web アプリは、SPA(Single Page Application) によるクライアントでの応答性を重視して発展してきました。近年、コンテンツが重要なサイトでは事前ビルドした静的コンテンツによる MPA(Multi Page Application) が注目されています。また、コンテンツの配信先を CDN のエッジ環境にしてホスト管理プロセスを無くす Jamstack なアーキテクチャが普及してきています。ここでは、プリレンダリングフレームワークや豆蔵デベロッパーサイトを配信しているホスティングサービス Netlify をはじめとするエッジの活用についてご紹介します。
...
## Deno
Deno は高速でセキュアな JavaScript ランタイムです。Node.js に比べると開発体験も向上しています。ここでは、Deno の導入や活用についてご紹介します。
