...
初めに [前のセクション](#1-so-101-の印刷もしくは購入) で用意したモーターを Leader と Follower 用に分けます。Leader 側は複数種類のギア比を持つモーターで構成されるのに対し、Follower 側は同一仕様（1 / 345）のモーターで構成されるためです。

Leader の各関節に割り当てるモーターID （[後述を参照](#23-モーターとモーターバスを連動させる)）とギア比は次の通りです。
...
[前セクション](#21-モーターの識別) で分けた Leader 用の全モーターに対して1台、Follower 用の全モーターに対して1台のモーターバスを用意します。このとき、Leader と Follower のモーターを誤って混同すると後で修正が大変なので、タックシールなどで全モーターにラベルを貼り、どのモーターバスに対応させるかを明確にするといいです。  
...
2. 各モーターバスの USB ポートを確認する。  
    ...
    両方のモーターバスを接続した状態で下記を実行します。  
    スクリプト実行中に指示が出たら、Leader_MB の USB ケーブルを抜いて Enter キーを押してください。
    ...
    `Reconnect the USB cable.` が表示された時点で Leader_MB のケーブルを差し直してください。上記例では `/dev/ttyACM0` が Leader_MB のポートとなるので、こちらを記録してください（前述のように、タックシールで識別すると管理がしやすくなります）。
...
`--robot.port=` には [MotorBus（モーターバス）のセットアップ](#22-motorbusモーターバスのセットアップ) で確認した Leader_MB のポートを入力します。確認しポートが`/dev/ttyACM0` 以外であれば、適切に変更を行ってから実行してください。  
...
1. [モーターの仕分け](#21-モーターの仕分け) で用意したモーターとその章のテーブルと照合して、該当する種類のモーターを準備する（例：gripper → ギア比 1 / 147 のモーター）
...
続けて他の Leader 用モーターににも同様に ID を設定してください。これら手順の一連の流れは下記の公式動画から確認できます。
...
Leader_MB の設定が終わったら、同じように Follower_MB の設定を行います。前述の通り、Follower 側のモーターは同一のギア比で構成されていることが多いので、モーターが担当する関節名を照合する必要はありません。ただ、設定した ID を混同しないよう、留意してください。


:::info:アドバイス
公式リポジトリの動画ではモーターを既にアームに取り付けた状態で ID 設定を行っていますが、筆者は取り付け前に ID を設定することをおすすめします。Leader 側は複数のギア比の異なるモーターで構成され、取り付け済みだと種類判別が難しく、誤った位置に取り付けると分解・ID 再設定が必要になるためです。  
...
1. サーボホーンにロボットアームの外装・機構パーツを固定する際は、大きなネジを使います。小さなネジはモーター本体をロボットアームに固定するために使われます。
