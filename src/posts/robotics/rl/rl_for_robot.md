---
title: ロボットのための強化学習入門 ~モデリングからDQNまで~
...
このタスクでは、台車に左右から力を加えることで、ポールのバランスを取り続けることを目指します。ポールが一定の角度以上に傾くと、または台車が画面の両端よりも外側に出ると失敗となり、その時点で終了します。タスクを開始してから終了するまでを、強化学習では**エピソード**という単位で表します。上のGIF画像は学習が不完全な状態であり、短時間でエピソードが終了し、リトライを繰り返している様子です。
...
システムへの入力を強化学習では**アクション**と呼びます。CartPoleタスクでは、アクションとして「左へカートを押す」と「右へカートを押す」のいずれかが選択できます。押す力は一定なので、PID制御のように出力を細かく調整できません。なお、この記事で扱うものとは別の手法を用いれば連続値のアクションを扱うことも可能です。
...
ここで$\gamma$は割引率といい、収益が発散することを防ぐ役割があります。[^touhi]
...
状態価値関数$V$を、収益$R_t$、現在の状態$s$を使って表すと次のようになります。

$$V^{\pi}(s) = \mathbb{E}_\pi\left[R_t| s_t = s \right] $$

また、行動価値関数$Q$は次のようになります。
...
期待値を計算するためには、「結果の値」×「その発生確率」を計算し、総和をとります。今回は確率的な要素が方策と状態遷移の2つあるため、総和を2回計算する必要があります。
まず「結果の値」を考えます。下図のようにある状態$s$からアクション$a_1$を選択し、次の状態$S^\prime$に遷移した場合の収益は次のように表現できます。

$$
r + \gamma V^\pi(s^\prime)
$$

これは現在の状態で得られる報酬と、それ以降の収益を分けて書いた表現になります。

![](/img/robotics/rl/backup6.png)

まず1つ目の総和として状態遷移確率の総和をとると、次のようになります。

$$
\sum_{s^\prime} p(s^\prime|s,a)\{r + \gamma V^\pi(s^\prime)\}
$$

同様に方策についても総和をとると、以下のようになります。この式は状態$s$のときの収益の期待値を表します。
...
- $r$: 報酬
- $\gamma V^\pi(s^\prime)$: 次のステップでの収益の期待値 $\times$ 割引率
...
例えば割引率$\gamma=0.95$を採用した橙線を見ると、100ステップ以上エピソードが続くときの収益は20になることがわかります。

次にバランスを崩してエピソードが終了する直前の価値を確認します。下の図に示すように、ポール角度がエピソードの終了条件である$12^\circ$のラインの少し内側にあり、そのラインを超える方向に向かって大きな角速度がある状態$s$を考えます。
...
$r$は状態$s$に遷移したときに与えられた報酬です。状態$s$に遷移した時点ではまだエピソードの終了条件に当てはまっていないため、報酬1を獲得します。次のステップ以降に獲得できる収益の期待値である$V^\pi(s^\prime)$は（選択したアクションによらずエピソードが終了する状態であるため）0です。その結果、上の図の状態$s$の状態価値は1になります。

さて、本題である価値関数の形を考えます。ただし状態価値関数の引数である状態$s$は4次元のベクトルなので可視化できません。ここではざっくりとしたイメージをお伝えしたいので、ポール角度のみを変数とし、それ以外のカート位置・カート速度・ポール角速度は0で固定したときの収益のグラフを作成します。
