...
また、動作間を曲線で補間でき、半径を指定することでなめらか度合いの指定が可能です。そのためマニュアルでロボットの軌道をチューニングする作業が行いやすいことも特徴の一つです。
...
PTP命令はロボットの手先が指定した座標に到達するようにロボットを移動させる命令です。動作開始点から到達点までの軌道がどのようになるかは問われません。
