...
今私が携わっているプロジェクトにおいてAWS Control Tower（以降Control Tower）を導入したアカウント運用の話があり、個人ではあまり使う機会が少ないサービスだと感じたので、この機会に使っていく中で検証した内容を書き留めておこうと思います。

そもそもControl Towerってどんなサービスなの？
AWS Control Towerは、AWSのベストプラクティスに基づいたマルチアカウント環境（ランディングゾーン）を自動的にセットアップし、継続的なガバナンスを提供するサービスです。

この記事で紹介するControl Towerで検証した内容は、Control Towerの「Account Factory」と「Account Factory Customization（以降AFC）」による「アカウント作成」の挙動についてです。
...
1.  **SearchProductsAsAdmin**: 所有者が「AWS Control Tower」であるService Catalog製品を検索します。
2.  **DescribeProduct**: 検索で見つかった製品（Account Factory）の詳細を取得します。
